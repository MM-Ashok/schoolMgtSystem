...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">All Pages</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-600 text-white rounded-tl rounded-tr">
                ...
                <div class="w-3/12 px-4 py-3">Slug</div>
            ...
            @foreach ($page as $item)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $item->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $item->slug }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $item->status == '1' ? 'Draft' : 'Published' }}</div>
                    <div class="w-3/12 flex items-center justify-end px-3">
                        <a href="{{ url('edit-page/'.$item->id)}}">
                        ...
                        <!-- <a href="{{ url('delete-page/'.$item->id)}}" data-url="#" class="deletebtn ml-1 bg-red-600 block p-1 border border-red-600 rounded-sm">
                        ...
                        <form method="POST" action="{{ url('delete-page/'.$item->id)}}">
                            @csrf
